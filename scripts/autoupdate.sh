...
set +e
wget https://github.com/HeliosLAM/nanopi-openwrt/releases/download/$(date +%Y-%m-%d)/$board_id$ver.img.gz -O $board_id.img.gz
if [ $? -eq 0 ]; then
	...
	wget https://github.com/HeliosLAM/nanopi-openwrt/releases/download/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.gz -O $board_id.img.gz
	if [ $? -eq 0 ]; then
